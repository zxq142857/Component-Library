<template>
  <div class="e-selector">
    <div class="e-input">
      <input type="text" id="Options" placeholder="请选择" class="e-selector-value" :value="selectorValue.join(' / ')" @focus="openOptions" autocomplete="off">
      <i class="e-selector-icon el-input__icon" :class="showOptions ? 'el-icon-arrow-up':' el-icon-arrow-down'"></i>
    </div>
    <div class="e-options-box" v-show="showOptions" id="e-selector-box">
      <ul class="e-options">
        <li v-for="(item,index) in firstOptions" :key="index" @click="showNextOptions(item,1)" id="Options" >
          <span id="Options">{{ item.name }}</span>
          <i class="el-icon-arrow-right el-cascader-node__postfix" id="Options"></i>
        </li>
      </ul>
      <ul class="e-options" v-for="(children,index) in nextOptions" :key="index">
        <li v-for="(item,keyid) in children" :key="keyid" @click="showNextOptions(item,index+2)" id="Options">
          <span id="Options">{{ item.name }}</span>
          <i class="el-icon-arrow-right el-cascader-node__postfix" v-if="item.hasSubset" id="Options"></i>
        </li>
      </ul>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    showNextOptions(item,index) {
      ...
      if(!item.hasSubset) {
        this.showOptions = false
        if(this.nextTier == this.currentTier + 1 || this.nextTier == this.currentTier || this.nextTier < this.currentTier) {
...
<style lang="less" scoped>
  ...
  .e-options-box:before,.e-options-box:after{
    position: absolute;
    content: "";
    width: 0;
    height: 0;
